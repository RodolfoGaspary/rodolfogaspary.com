...
const Logo: React.FC<LogoProps> = ({ className = "size-8" }) => {
  ...
  const FallbackLogo = () => (
    <div className={`${className} text-primary transition-transform duration-500 group-hover:scale-110`}>
  ...
  const MonogramLogo = () => (
    <div className={`${className} flex items-center justify-center transition-transform duration-500 group-hover:scale-110`}>
      <svg viewBox="0 0 653 653" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
        <path fill="#135bec" d="m321.85 617.79c-155.86 0-281.85-131.68-281.85-294.59 0-162.91 125.99-294.6 281.85-294.6 155.86 0 281.85 131.69 281.85 294.6 0 162.91-125.99 294.59-281.85 294.59z"/>
